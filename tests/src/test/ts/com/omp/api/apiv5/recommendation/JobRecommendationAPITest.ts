...
export class JobRecommendationAPITest extends BaseRestTest {

    private user1RequestContext: APIRequestContext;
    private user2RequestContext: APIRequestContext;

    public initialize(): void {
      this.user1RequestContext = this.loginToAUserForInternalRequest(this.getUserByName("user1"));
    ...
    public checkNoRecommendationsForJobs(): void {
        let response: APIResponse = this.postInternalRequest(this.user1RequestContext, EndpointsEnum.JOB_RECOMMENDATION.getEndpoint(), "{}");
    ...
    public checkExcellentRecommendationForJobs(): void {
        let response: APIResponse = this.postInternalRequest(this.user2RequestContext, EndpointsEnum.JOB_RECOMMENDATION.getEndpoint(), "{}");

        this.apiAssertions.assertStatus(response, 200);
        this.apiAssertions.assertEqual(response, "[0]/title", "Financial planning and analysis manager");
    ...
    public checkGoodRecommendationForJobs(): void {
        ...
        this.apiAssertions.assertStatus(response, 200);
        this.apiAssertions.assertEqual(response, "[1]/title", "Financial planning and analysis manager - good");
    ...
    public checkFairRecommendationForJobs(): void {
        ...
        this.apiAssertions.assertStatus(response, 200);
        this.apiAssertions.assertEqual(response, "[2]/title", "Financial planning and analysis manager - fair");
