...
export class RequestMentorshipAcceptAsMentorTest extends BaseRestTest {
    ...
    public AcceptMentorshipRequestAndValidateStatus(): void {
        this.getOmpLoginPage()
                ...
                .endAssertion()
                .clickAcceptRejectButton(String.valueOf(this.user.name), "Accept")
                .selectAFilterOption("mentor-options-APPROVED")
                //.waitForMentorshipCardToBeVisible()
                .check(MyMentorshipAssertions)
                    .assertMentorApplicationStatusIsDisplayed(String.valueOf(this.user.name),"Accepted")
