...
export class VerifyBrowseBySubjectTest_CSX extends BaseTest {
	...
	public verifyBrowseAndClearAllOption(): void {
		...
		.loginToThinkContent(this.getPlayOneUser().email, this.getPlayOneUser().password)
	...
	public verifyBadgesNew(): void {
		...
		.check(BrowseBySubjectAssertions)
		.assertThatBadgeIsVisible(this.NEW_BADGE_COURSE.getValue(), this.LABEL_SUBSCRIBE)
	...
	public verifyBadgesRetiring(): void {
		...
		.waitForCard()
		.check(BrowseBySubjectAssertions)
		.assertThatBadgeIsVisible(this.LABEL_SEARCHCOURSE_RETIRE, this.LABEL_SUBSCRIBE)
